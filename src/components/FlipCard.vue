<template>
  <div :class="{'flip-card': true}">
    <div
      ...
      v-bind:style="{transform: 'rotateY(' + degree + 'deg)', '-webkit-transform': 'rotateY(' + degree + 'deg)', '-ms-transform': 'rotateY(' + degree + 'deg)'}">
      <div :class="{'flip-card-front flex column items-center': true, 'bordered': !borderless}">
        <div class="btn-flip">
          <q-btn v-if="!hideFlipButton" rounded dense color="warning" class="q-mr-sm" icon="refresh" @click="flip"/>
          <q-btn v-if="!hidePrintButton" rounded dense color="warning" icon="print" @click="print"/>
        </div>
...
@media print and (orientation: portrait) {
  .q-page .bg-theme {min-height: 100% !important;}
  .btn-flip {display: none;}
  .flip-card{top: 0 !important;}
  .flip-card-front,.flip-card-back{position: relative !important;}
  .flip-card-back{transform: rotateY(0) !important;}
  .flip-card{-webkit-print-color-adjust: exact;}
}
